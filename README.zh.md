...
**APKCook** 是一款专业级 Android 逆向与安全分析 GUI，基于 Avalonia (.NET 8) 构建。它将 `apktool` 的强大能力与高级静态分析功能结合，并以高性能赛博朋克风格界面呈现。APKCook 从反编译到分析、重建与签名，贯穿整个工作流。

[观看 YouTube 演示](https://youtu.be/Mkdt0c-7Wwg)

APKCook 采用单窗口工作流，并在顶部提供各工具导航：**Decompile**、**Build**、**Analyser**、**Settings** 和 **About**。每个部分对应 APK 生命周期的一个阶段，让你无需离开应用即可从解码切换到分析和签名。
...
### 安全分析
APKCook 内置静态分析器，可扫描反编译代码中的常见安全指标：
...
4.  **.NET 8.0 Runtime**：在受支持的平台（Windows、Linux、macOS）上运行 APKCook 所需。
...
### 1) 反编译流程
![APKCook 反编译界面](images/pulse_apk_decompile.png)
- 该界面用于选择输入 APK 和输出目录，然后执行反编译。
- 简单流程：选择 APK -> 设置输出路径 -> 点击 decompile。

### 2) 构建流程
![APKCook 构建界面](images/pulse_apk_build.png)
- 该界面用于将反编译项目重新构建为新的 APK。
- 简单流程：选择项目目录 -> 设置输出名称/路径 -> 点击 build（需要时启用签名）。

### 3) 静态分析结果
![APKCook 分析输出](images/pulse_apk_analysis.png)
- 该视图展示 Smali/静态分析得到的安全问题结果。
- 简单流程：先反编译 -> 打开分析标签页/输出 -> 查看结果并导出报告。

## 技术架构

APKCook 使用清晰的 MVVM（Model-View-ViewModel）架构：
...
如果 APKCook 对你有帮助，可以点击顶部的 “Support” 按钮支持其开发。
