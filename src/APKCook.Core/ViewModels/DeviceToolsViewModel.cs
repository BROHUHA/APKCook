...
public partial class DeviceToolsViewModel : ObservableObject
{
    ...
    private async Task TakeScreenshot()
    {
        ...
        var remotePath = $"/sdcard/APKCook-screenshot-{timestamp}.png";
        var localPath = Path.Combine(outputFolder, $"APKCook-screenshot-{timestamp}.png");
    ...
    private async Task StartScreenRecord()
    {
        ...
        var timestamp = DateTime.Now.ToString("yyyyMMdd-HHmmss");
        var remotePath = $"/sdcard/APKCook-recording-{timestamp}.mp4";
        var localPath = Path.Combine(outputFolder, $"APKCook-recording-{timestamp}.mp4");
