...
namespace APKCook.Tests.Services.Patching;
...
public class PatchPipelineServiceTests
{
    ...
    public async Task RunAsync_UsesRootMarkerForFinalDexVerification_WhenRootBypassProfileIsEnabled()
    {
        ...
        Assert.Equal("/dev/apkcook-fake-root-", fakeFinalDexInspectionService.LastMarkerLiteral);
        var stage = Assert.Single(result.StageSummaries.Where(static s => s.Stage == "dex-verification"));
    ...
    public async Task RunAsync_UsesRootSpecificGuidance_WhenRootMarkerMissingAfterSearchCompletion()
    {
        ...
        await File.WriteAllTextAsync(inputApk, "apk");
        var outputApk = Path.Combine(Path.GetTempPath(), $"output-{Guid.NewGuid():N}.apk");
        var diagnostics = "Marker literal '/dev/apkcook-fake-root-' not found in any of the 2 dex entries.";
        var pipeline = CreatePipeline(
    ...
    public async Task RunAsync_VerifiesApplicationHelper_WhenActivityPatchFallbackBranchIsUsed()
    {
        ...
        Assert.True(result.Success);
        Assert.Equal(
            "Lcom/apkcook/generated/PulseFridaApplication;->loadFridaGadgetSafely()V",
            fakeFinalDexInspectionService.LastMethodReference);
    ...
    private sealed class FakeApktoolService : IApktoolService
    {
        ...
        public Task<ApktoolRunResult> DecompileAsync(string apkPath, string outputDirectory, bool decodeResources, bool decodeSources, CancellationToken cancellationToken = default)
        {
            ...
            {
                return Task.FromResult(new ApktoolRunResult(_decompileExitCode, [], ["decompile failed"]));
            }
            ...
            return Task.FromResult(new ApktoolRunResult(0, [], []));
        }
        ...
        public Task<ApktoolRunResult> BuildAsync(string decompiledDirectory, string outputApkPath, bool useAapt2, CancellationToken cancellationToken = default)
        {
            if (_buildExitCode != 0)
            {
                return Task.FromResult(new ApktoolRunResult(_buildExitCode, [], ["build failed"]));
            }
    ...
    private sealed class FakeFinalDexInspectionService : IFinalDexInspectionService
    {
            ...
            string? markerDiagnostics = null)
        {
            ...
            _markerDiagnostics = markerDiagnostics ?? (_containsMarker ? "Marker literal '/dev/apkcook-fake-root-' found in fake dex." : "Marker literal '/dev/apkcook-fake-root-' missing in fake dex.");
        }
