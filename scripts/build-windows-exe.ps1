param(
    ...
    [string]$AppName = $(if ($env:APP_NAME) { $env:APP_NAME } else { "APKCook" }),
...
$projectPath = Join-Path $repoRoot "src/APKCook.Avalonia/APKCook.Avalonia.csproj"

$appExe = "$AppName.exe"

$outRoot = Join-Path $repoRoot "artifacts/windows/$Rid"
$publishDir = Join-Path $outRoot "publish"
$zipPath = Join-Path $outRoot "APKCook-$Rid.zip"
