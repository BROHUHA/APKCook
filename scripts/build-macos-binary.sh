...
project_path="${repo_root}/src/APKCook.Avalonia/APKCook.Avalonia.csproj"
macos_icon_path="${repo_root}/Resources/APKCook.icns"
macos_icon_name="APKCook"
macos_icon_file="${macos_icon_name}.icns"

config="${CONFIGURATION:-Release}"
rid="${RID:-osx-arm64}"
app_name="${APP_NAME:-APKCook}"
bundle_name="${APP_BUNDLE_NAME:-APKCook}"
app_exe="${app_name}"
...
if [[ "${rid}" != "osx-arm64" ]]; then
  echo "APKCook macOS releases are arm64 only. Set RID=osx-arm64 (received '${rid}')." >&2
  exit 1
fi
...
if [[ ! -f "${publish_dir}/${app_exe}" ]]; then
  ...
  echo "Expected '${APP_NAME:-APKCook}' was not found; using discovered executable '${app_exe}'."
fi
...
<dict>
  ...
  <string>com.apkcook.${bundle_identifier_name}</string>
  ...
  <string>APKCook needs access to APK files and output folders you select on the Desktop so it can decompile and rebuild APK projects.</string>
  <key>NSDocumentsFolderUsageDescription</key>
  <string>APKCook needs access to APK files and output folders you select in Documents so it can decompile and rebuild APK projects.</string>
  <key>NSDownloadsFolderUsageDescription</key>
  <string>APKCook needs access to APK files and output folders you select in Downloads so it can decompile and rebuild APK projects.</string>
  <key>NSRemovableVolumesUsageDescription</key>
  <string>APKCook needs access to APK files and output folders you select on removable volumes so it can decompile and rebuild APK projects.</string>
  <key>NSNetworkVolumesUsageDescription</key>
  <string>APKCook needs access to APK files and output folders you select on network volumes so it can decompile and rebuild APK projects.</string>
</dict>
</plist>
PLIST
